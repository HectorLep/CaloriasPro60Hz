...
class ProfileWidget(QWidget):
    ...
    def init_ui(self):
        ...
        # Estilo para hacer la imagen circular y con borde
        self.profile_image.setStyleSheet("""
            QLabel {
                border: 3px solid #4CAF50;
                border-radius: 40px; /* La mitad del tamaño */
    ...
    def select_photo(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            ...
            # ESTA OPCIÓN SOLUCIONA EL CRASH EN LINUX CON CONDA
    ...
    def set_picture(self, image_path):
        """Carga y muestra una imagen en el label circular."""
        if image_path and os.path.exists(image_path):
            pixmap = QPixmap(image_path)
            self.profile_image.setPixmap(pixmap.scaled(80, 80, Qt.AspectRatioMode.KeepAspectRatioByExpanding, Qt.TransformationMode.SmoothTransformation))
        else:
            self.profile_image.setText("👤")
            self.profile_image.setStyleSheet(self.profile_image.styleSheet() + "font-size: 40px; color: #ccc;")

class NavigationButton(QPushButton):
    """Botón de navegación personalizado (tu código original)."""
...
class Sidebar(QWidget):
    """Barra lateral principal, ahora consciente del usuario."""
    ...
    def create_navigation_buttons(self, layout):
        buttons_data = [
            ("Registrar Alimento", "📝", "registrar"), ("Agregar Alimento", "➕", "agregar"),
            ("Gráfico", "📊", "grafico"), ("Historial", "🕐", "historial"),
            ("Settings", "⚙️", "settings"), ("Salud", "🛡️", "salud"),
            ("Menu", "📋", "menu")
        ]
